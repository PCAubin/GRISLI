function [AUROC_ida, AUPR_ida, elapsedTime,...
    TPR_array_IDA, FPR_array_IDA, PPV_array_IDA,I_ida]=Test_MI_realdata(filenumber,A)
% Test_MI_realdata computes the AUROC scores of the matrices predicted
% based on the mutual inference (package infotheo) as compared to the 
% reference matrix A (G*G) for the given dataset found at data*filenumber*/.
% The reference matrix is symmetrized as mutual inference is symmetrical
% and cannot infer symmetrized edges.
% Unlike the SCODE testing procedure where R was called within Matlab, the
% R files to run MI are to be found in SCODE-master. The follwing code
% only reads the pre-computed matrices without doing any computation.
% Pierre-Cyril Aubin-Frankowski, 2020

tic
A_IDA = dlmread(['SCODE-master/data' num2str(filenumber) '/A_MI.txt'],'\t');
elapsedTime = toc;
...
A_wo_diag_red=(A_wo_diag_red+A_wo_diag_red')>0;
A_IDA = tril(A_IDA - diag(diag(A_IDA)));
[~,I_ida]=sort(abs(A_IDA(:)),'descend');
[~,I_ida]=sort(I_ida);
I_ida=reshape(I_ida,size(A_IDA));

% Rnk_array_SCODE=zeros(size(A_IDA,1),size(A_IDA,2),numel(A));
TPR_array_IDA=zeros(1,length(numel(A)+1));
FPR_array_IDA=zeros(1,length(numel(A)+1));
PPV_array_IDA=zeros(1,length(numel(A)+1));
for k=1:size(A,1)*(size(A,1)-1)/2
   A_app_Rnk=(I_ida<k);
   A_app_Rnk=(A_app_Rnk+A_app_Rnk')>0;
...
%     fprintf(fileID,fmt, [size(X,1) number_tries number_average AUROC_MI elapsedTime]);
%     fclose(fileID);
% end
end
